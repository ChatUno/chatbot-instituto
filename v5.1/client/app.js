...
class VisualRAGApp {
  ...
  initializeEventListeners() {
    ...
    document.getElementById('mode-crawl')?.addEventListener('click', () => this.switchMode('crawl'));
    
    // Crawl functionality
    document.getElementById('crawl-btn')?.addEventListener('click', () => this.startCrawling());
  ...
  switchMode(mode) {
    ...
    if (mode === 'single') {
      ...
      document.getElementById('mode-crawl').classList.add('active');
      // Hide single page workflow
      document.querySelectorAll('.step-section').forEach(section => {
  ...
  async startCrawling() {
    ...
    const maxPages = parseInt(document.getElementById('crawl-max-pages').value);
    const maxDepth = parseInt(document.getElementById('crawl-depth').value);
    const delayMs = parseInt(document.getElementById('crawl-delay').value);
    ...
    if (!this.validateURL(url)) {
      this.showToast('La URL debe ser válida y empezar con http:// o https://', 'error');
      return;
    }
    ...
    crawlBtn.textContent = 'Crawling en progreso...';
    crawlStatus.innerHTML = '<div class="status-message info">Iniciando crawl de ' + url + '<br>Esto puede tardar varios minutos.</div>';
    crawlProgress.innerHTML = '<div class="loading-spinner"></div>';
    ...
    try {
      ...
      if (data.success) {
        const d = data.data;
        crawlStatus.innerHTML = `<div class="status-message success">Crawl completado: ${d.pagesSuccess} páginas procesadas, ${d.pagesError} errores, ${d.chunksGenerated} chunks generados</div>`;
        
        // Render results table
        this.renderCrawlResults(d.results, d.errors);
        
        // If chunks were generated, show them for approval
        if (d.chunksGenerated > 0) {
          this.state.chunks = d.chunks.map(chunk => ({
            ...chunk,
            status: 'pending'
          }));
          this.displayCrawlChunks();
          document.getElementById('export-section').style.display = 'block';
        }
        
        this.showToast('Crawling completado exitosamente', 'success');
      } else {
      ...
      crawlStatus.innerHTML = `<div class="status-message error">Error: ${error.message}</div>`;
      this.showToast('Error en el crawling: ' + error.message, 'error');
    } finally {
      // Re-enable button
      crawlBtn.disabled = false;
      crawlBtn.textContent = 'Iniciar crawling';
      crawlProgress.innerHTML = '';
    }
  }

  renderCrawlResults(results, errors) {
    const crawlResults = document.getElementById('crawl-results');
    
    let html = `
      <div class="crawl-results">
        <h4>Resultados del Crawling</h4>
    ...
    results.forEach(result => {
      ...
      const statusText = result.status === 'success' ? 'Éxito' : 
                        result.status === 'error' ? 'Error' : 'Omitido';
      
      html += `
        <tr class="${statusClass}">
          <td>${this.truncateUrl(result.url, 50)}</td>
          <td>${statusText}</td>
          <td>${result.chunksGenerated || 0}</td>
          <td>${result.linksFound || 0}</td>
        </tr>
      `;
    });
    ...
    if (errors.length > 0) {
      html += `
        <div class="crawl-errors">
          ...
          <ul>
            ${errors.map(error => `<li><strong>${this.truncateUrl(error.url, 40)}:</strong> ${error.error}</li>`).join('')}
          </ul>
        </div>
      `;
    }
    ...
    html += `</div>`;
    crawlResults.innerHTML = html;
  }

  displayCrawlChunks() {
    const chunksList = document.getElementById('chunks-list');
    if (!chunksList) {
      // Create chunks list if it doesn't exist
      const container = document.createElement('div');
      container.id = 'chunks-list';
      container.className = 'chunks-list';
      document.getElementById('crawl-section').appendChild(container);
    }
    
    this.renderChunks();
    document.getElementById('chunks-list').style.display = 'block';
  }
